...
function replacePrivateKey () {
    ...
    CURRENT_DIR=$PWD
    cd ./$CRYPTO_CONFIG_LOCATION/peerOrganizations/org.simple.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd $CURRENT_DIR

    sed $OPTS "s/CA0_PRIVATE_KEY/${PRIV_KEY}/g" "docker-compose-fabric-${VERSION_DIR}.yaml"

    cd ./$CRYPTO_CONFIG_LOCATION/peerOrganizations/org2.hoojo.cn/ca/
    PRIV_KEY=$(ls *_sk)
    cd $CURRENT_DIR

    sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" "docker-compose-fabric-${VERSION_DIR}.yaml"

    cd ./$CRYPTO_CONFIG_LOCATION/peerOrganizations/org3.example.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd $CURRENT_DIR
...
function generateAnchorPeerArtifacts() {
    ...
    echo "#################################################################"
    log yellow "==> cryptogen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./$CHANNEL_ARTIFACTS_LOCATION/OrgMSPanchors.tx -channelID $1 -asOrg OrgMSP"
    $CONFIGTXGEN -profile TwoOrgsChannel -outputAnchorPeersUpdate ./$CHANNEL_ARTIFACTS_LOCATION/OrgMSPanchors.tx -channelID $1 -asOrg OrgMSP
    
    log done "generate anchor peer[OrgMSP]"
    echo "#################################################################"
    ...
    echo "#################################################################"
    log yellow "==> cryptogen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./$CHANNEL_ARTIFACTS_LOCATION/Org3MSPanchors.tx -channelID $1 -asOrg Org3MSP"
    $CONFIGTXGEN -profile TwoOrgsChannel -outputAnchorPeersUpdate ./$CHANNEL_ARTIFACTS_LOCATION/Org3MSPanchors.tx -channelID $1 -asOrg Org3MSP
    
    log done "generate anchor peer[Org3MSP]"
...
function fetchRequiredChannelArtifacts() {
    ...
    requiredFiles="crypto-config/peerOrganizations/org.simple.com/msp/cacerts"
